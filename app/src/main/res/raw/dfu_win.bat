...
if "%ADDRESS%"=="" (
 ...
 -e no.nordicsemi.android.extra.dfu.EXTRA_FILE_PATH "/sdcard/Nordic Semiconductor/%HEX_FILE%" | find "unable" > nul 2>&1
	if errorLevel 0 (
		echo FAIL
 ...
 -e no.nordicsemi.android.extra.dfu.EXTRA_DEVICE_ADDRESS %ADDRESS%^
 -e no.nordicsemi.android.extra.dfu.EXTRA_DEVICE_NAME %NAME%^
 -e no.nordicsemi.android.extra.dfu.EXTRA_FILE_PATH "/sdcard/Nordic Semiconductor/%HEX_FILE%" > nul 2>&1
	if errorLevel 1 (
		echo FAIL
		echo Error: Required application not installed.
		goto error
