...
# Start DFU Initiator activity if no target device specified
    ...
    adb $S_DEVICE shell am start -a no.nordicsemi.android.action.DFU_UPLOAD -e no.nordicsemi.android.extra.dfu.EXTRA_FILE_PATH "/sdcard/Nordic Semiconductor/$HEX_FILE" | grep "unable" > /dev/null 2>&1
    if [ "$?" = "0" ] ; then
        echo "FAIL"
    ...
    adb $S_DEVICE shell am startservice -a no.nordicsemi.android.action.DFU_UPLOAD -e no.nordicsemi.android.extra.dfu.EXTRA_DEVICE_ADDRESS $ADDRESS -e no.nordicsemi.android.extra.dfu.EXTRA_DEVICE_NAME $NAME -e no.nordicsemi.android.extra.dfu.EXTRA_FILE_PATH "/sdcard/Nordic Semiconductor/$HEX_FILE" | grep "unable" > /dev/null 2>&1
    if [ "$?" = "0" ] ; then
        echo "FAIL"
        echo "Error: Required application not installed."
        error
